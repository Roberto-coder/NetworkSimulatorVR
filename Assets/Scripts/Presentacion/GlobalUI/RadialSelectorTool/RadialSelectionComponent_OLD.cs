...
{
    public class RadialSelectionComponent_OLD : MonoBehaviour
    {
        [Header("UI")]
        ...
        private int RadialPartCount =>
            toolManager.AvailableTools.Count;
        ...
        void Update()
        {
            ...
            if (menuActive && input.ToolHeld)
                UpdateSelection();

            if (menuActive && input.ToolReleased)
                ConfirmSelection();
        }
        ...
        void ShowMenu()
        {
            ...
            if (toolManager.AvailableTools.Count == 0)
            {
        ...
        void ConfirmSelection()
        {
            ...
            {
                toolManager.EquipTool(toolManager.AvailableTools[currentIndex]);
                LogDebug($"Equipped tool index {currentIndex}: {(toolManager.AvailableTools[currentIndex] == null ? "NULL" : toolManager.AvailableTools[currentIndex].name)}");
            }
        ...
        void SpawnParts()
        {
            ...
            int partCount = toolManager.AvailableTools.Count;
            if (partCount == 0)
            {
            ...
            for (int i = 0; i < partCount; i++)
            {
                ...
                if (toolManager.AvailableTools[i] != null)
                    SetupIcon(part.transform, toolManager.AvailableTools[i], fillAngle, centerAngle);
